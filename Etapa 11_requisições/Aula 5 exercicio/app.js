...
*/
const threeArguments = (one, two, three) => {
  const isSomeParameterUndefined = [one, two, three].includes(undefined)
...
const getAvaliabeSpacesMessage = (spaces, booksIn) => {
  ...
  const bookPluralSingular = 
    getPluralOrSingular(avaliableSpaces, 'livro', 'livros')
  return `Só ${fitPluralOrSingular} mais ${avaliableSpaces} ${bookPluralSingular}`
}
...
booksBox.addBooks = booksQuantity => {
  ...
  return `Já ha ${booksBox.booksIn} ${bookPluralOrSingular} na caixa`
}
