...
console.log(getIndex('o', 'krankenhouse'));
/*
  05
...
*/
const removeArrayItem = value => {
  value.pop()
  return value
}

console.log(removeArrayItem([1, 2, 3, 4]));
/*
  08
...
const invokeMyName = () => console.log('elder fausto');

invokeCallback(invokeMyName)
/*
  10
...
let paragraphs = ''
const createParagraphs = (itemParagraph) => {
  paragraphs += `<p>${itemParagraph}</p>`
}
...
const peopleLikes = (likes = []) => {
  const firstName = likes[0]
  const secondName = likes[1]
  const thirdName = likes[2]
  const totalNamesMinusTwo = likes.length - 2
...
console.log(peopleLikes(['elder', 'pedro', 'vitoria', 'jao', 'maria']));