...
let bestMovies = "Segundo o site Omelete, os melhores filmes de 2019 são: ";

const allMovies2019 = (movies) => {
  bestMovies += ` 
  -  ${movies.title}, dirigido por ${movies.directedBy}. `;
};

best2019Movies.forEach(allMovies2019);
...
const youtubeUser = {
  ...
  getRecentVideosRogerMelo() {
    this.videos.recentVideos.forEach((title) => {
      console.log(title);
    });

    return `moviesRogerMelo`;
  },
};

youtubeUser.getRecentVideosRogerMelo();
...
console.log(Math.trunc(randomNumber * 10));
